function [new_estimation_state, errorAngleIncl, errorAngleAzi] = CSG_OriEst_IMU(estimation_state, Acc, Gyro, Mag, sample_freq, TauAcc, TauMag, Zeta, useMeasRating)
    ...
    % Further reading: T. Seel, S. Ruppin. Eliminating the Effect of Magnetic Disturbances on the Inclination Estimates of Inertial Sensors. IFAC-PapersOnLine,
    %   50 (1):8798–8803, 2017. (http://dx.doi.org/10.1016/j.ifacol.2017.08.1534)
    ...
    %     First, choose a start quaternion[4] and an initial bias[3] to begin the estimation with.
    ...
    %     
    %     Choose for TauAcc the time that should pass in seconds before the inclination disagreement between gyro-based prediction and acc measurements is halved.
    %     Analogously for TauMag.
    %     Choose for Zeta a value >= 0. This value will define your bias estimation aggressiveness. Zeta = 0 means no bias estimation will be done.
    ...
    TauAcc = min((validAccDataCount)/sample_freq/2,TauAcc);                                           % Speed-up acceleromter-based correction in the first samples
    TauMag = min((validAccDataCount)/sample_freq/2,TauMag);                                           % Speed-up magnetometer-based correction in the first samples
